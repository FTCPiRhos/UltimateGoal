...
@Autonomous(name="Auto Red Region", group="PiRhos")

public class AutoRedRegion extends AutoBaseRegional {
    class SetRPMVars {
    ...
    private AutoRedRegion.SetRPMVars shooterRPMVars = new AutoRedRegion.SetRPMVars();
    ...
    private AutoRedRegion.MoveWPIDVars moveWPIDVars = new AutoRedRegion.MoveWPIDVars();
    // target shooter speed
    double targetRPMGoal = -170;
    ...
    int loopCount = 0;
    double rightPwrMul = 1.0;
    double leftPwrMul = 0.95;
    ...
    public void runOpMode() {
        ...
        if (pipeline.getOrangeCt() > 0){
            telemetry.addData("red avg = ", pipeline.getTotalRed()/pipeline.getOrangeCt());
            telemetry.addData("green avg = ", pipeline.getTotalGreen()/pipeline.getOrangeCt());
            telemetry.addData("blue avg = ", pipeline.getTotalBlue()/pipeline.getOrangeCt());
        }
        telemetry.update();
        ...
        OpenCVPipelineShoot.StartRobotPosition startRobotPosition = pipeline.getInitRobotPos();
        telemetry.addData("Object Found: ", objectFound);
        telemetry.addData("start pos = ", startRobotPosition);
        if (pipeline.getOrangeCt() > 0){
            telemetry.addData("red avg = ", pipeline.getTotalRed()/pipeline.getOrangeCt());
            telemetry.addData("green avg = ", pipeline.getTotalGreen()/pipeline.getOrangeCt());
            telemetry.addData("blue avg = ", pipeline.getTotalBlue()/pipeline.getOrangeCt());
        }
        // telemetry.addData("height : ", pipeline.)
        telemetry.update();
        ...
        while (opModeIsActive()) {
            ...
            if (objectFound.equals(OpenCVPipelineShoot.RingPosition.NONE)){
                if (commandCount == 0){
                    // move forward 50
                    moveWPIDnew(0,-54,0.5);
                    // increment command when movement done
                ...
                if (commandCount == 1){
                    //shoot
                    startRobotPosition = aimShoot(targetRPMGoal,flywheelPower,true);
                    commandCount++;

                }

                if (commandCount == 2){
                    turnToZero(startRobotPosition);
                    commandCount++;
                }
                if (commandCount == 3){
                    if  (startRobotPosition.equals(OpenCVPipelineShoot.StartRobotPosition.LEFT)){
                        //left side code
                        moveWPID(0,-12,0.5);
                        rotateOdo(90,0.4);
                        moveWPID(0,-22,0.5);
            ...
            if (objectFound.equals(OpenCVPipelineShoot.RingPosition.ONE)){
                if (startRobotPosition.equals(OpenCVPipelineShoot.StartRobotPosition.LEFT)){
                    if (commandCount == 0){
                        moveWPIDnew(0,-54,0.7);
                    }
                    if (!moveWPIDVars.inMove){
                        commandCount++;
                        sleep(250);
                    ...
                    if (commandCount == 1){
                        aimShoot(targetRPMGoal,flywheelPower,true);
                        commandCount++;
                    }
                    ...
                    if (commandCount == 4){
                        ...
                        sleep(250);
                        moveWPID(0,-16,0.7);
                        aimShoot1x(targetRPMGoal,flywheelPower,true);
                        intakeOff();
                        commandCount++;
                    }
            ...
            if (objectFound.equals(OpenCVPipelineShoot.RingPosition.FOUR)) {
                if (startRobotPosition.equals(OpenCVPipelineShoot.StartRobotPosition.LEFT)) {
                    if (commandCount == 0) {
                        moveWPIDnew(0, -54, 0.85);
                    }
                    if (!moveWPIDVars.inMove) {
                        commandCount++;
                        sleep(50);
    ...
    public void moveWPIDnew(double targetXInches, double targetYInches, double maxPwr) {
        ...
        if (!moveWPIDVars.inMove) {
            ...
            if ((!moveWPIDVars.movementDoneX || !moveWPIDVars.movementDoneY)) {
                ...
                backRight.setPower(moveWPIDVars.curPowerRB * rightPwrMul);
                backLeft.setPower(moveWPIDVars.curPowerLB * leftPwrMul);

                frontLeft.setPower(moveWPIDVars.curPowerLF * leftPwrMul);
                frontRight.setPower(moveWPIDVars.curPowerRF * rightPwrMul);
